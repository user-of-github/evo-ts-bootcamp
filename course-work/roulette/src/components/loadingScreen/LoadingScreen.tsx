...
export const LoadingScreen = observer((props: { state: SettingsState }) => (
    <>
        <Transition in={props.state.showLoadingScreen || props.state.loading} unmountOnExit={true} timeout={1000} key={props.state.loading.toString()}>
            {
                state => (
                    <div className={Style.wrapper} style={{
                        ...
                        {!props.state.loading && (
                            <button className={Style.playButton} onClick={() => {
                                ...
                                props.state.showLoadingScreen = false;
                            }}>Play</button>
                        )}
                    </div>
                )
